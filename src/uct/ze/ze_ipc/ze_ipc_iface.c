...
//#include <uct/ze/base/ze_nvml.h> //todo: don't have this one on xpu
#include <uct/ze/ze_ipc/ze_ipc_device.h>
...
} uct_ze_ipc_device_addr_flags_t;


typedef struct {
    uint64_t     system_uuid;
    uint8_t      flags; /* uct_ze_ipc_device_addr_flags_t */
} UCS_S_PACKED uct_ze_ipc_device_addr_t;
...
static ucs_config_field_t uct_ze_ipc_iface_config_table[] = {
    ...
    {"MAX_POLL", "16",
     "Max number of event completions to pick during ze events polling",
      ucs_offsetof(uct_ze_ipc_iface_config_t, params.max_poll), UCS_CONFIG_TYPE_UINT},

    {"MAX_STREAMS", UCS_PP_MAKE_STRING(UCT_CUDA_IPC_MAX_PEERS),
     "Max number of CUDA streams to make concurrent progress on",
      ucs_offsetof(uct_ze_ipc_iface_config_t, params.max_streams), UCS_CONFIG_TYPE_UINT},

    {"CACHE", "y",
     "Enable remote endpoint IPC memhandle mapping cache",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.enable_cache),
     UCS_CONFIG_TYPE_BOOL},

    {"ENABLE_GET_ZCOPY", "auto",
     "Enable get operations except for platforms known to have slower performance",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.enable_get_zcopy),
     UCS_CONFIG_TYPE_ON_OFF_AUTO},

    {"MAX_EVENTS", "inf",
     "Max number of ze events. -1 is infinite",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.max_ze_ipc_events), UCS_CONFIG_TYPE_UINT},

    {"BW", "auto",
     "Effective p2p memory bandwidth",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.bandwidth), UCS_CONFIG_TYPE_BW},

    {"LAT", "1.8us",
     "Estimated latency",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.latency), UCS_CONFIG_TYPE_TIME},

    {"OVERHEAD", "4.0us",
     "Estimated CPU overhead for transferring GPU memory",
     ucs_offsetof(uct_ze_ipc_iface_config_t, params.overhead), UCS_CONFIG_TYPE_TIME},
    {NULL}
};
...
ucs_status_t uct_ze_ipc_iface_get_device_address(uct_iface_t *tl_iface,
                                                   uct_device_addr_t *addr)
{
    uct_ze_ipc_device_addr_t *dev_addr = (uct_ze_ipc_device_addr_t*)addr;
    uct_ze_ipc_iface_t *iface          = ucs_derived_of(tl_iface,
                                                          uct_ze_ipc_iface_t);
    uct_ze_ipc_md_t *md                = ucs_derived_of(iface->super.super.md,
                                                           uct_ze_ipc_md_t);
...
static ucs_status_t uct_ze_ipc_iface_get_address(uct_iface_h tl_iface,
...
static int
uct_ze_ipc_iface_mnnvl_supported(uct_ze_ipc_md_t *md,
                                   const uct_ze_ipc_device_addr_t *dev_addr,
                                   size_t dev_addr_len)
{
    if (md->enable_mnnvl && (dev_addr_len != sizeof(uint64_t))) {
        ucs_assertv(dev_addr_len >= sizeof(uct_ze_ipc_device_addr_t),
                                   ...
                                   const uct_iface_is_reachable_params_t *params)
{
    ...
    dev_addr     = (const uct_ze_ipc_device_addr_t *)params->device_addr;
    ...
    if (same_uuid ||
        uct_ze_ipc_iface_mnnvl_supported(md, dev_addr, dev_addr_len)) {
...
static double uct_ze_ipc_iface_get_bw() //todo: no need to check bw
{
    CUdevice cu_device;
    int major_version;
    ucs_status_t status;
...
//static int uct_cuda_ipc_get_device_nvlinks(int ordinal) // todo: no need to check nvlink now
//{
...
static size_t uct_ze_ipc_iface_get_max_get_zcopy(uct_ze_ipc_iface_t *iface)
{
    return ULONG_MAX;
}

static ucs_status_t uct_ze_ipc_iface_query(uct_iface_h tl_iface,
                                             uct_iface_attr_t *iface_attr)
{
    ...
    iface_attr->device_addr_len         = md->enable_mnnvl ?
                                          sizeof(uct_ze_ipc_device_addr_t) :
                                          sizeof(uint64_t);
    ...
    iface_attr->cap.get.max_zcopy       = uct_ze_ipc_iface_get_max_get_zcopy(iface);
...
static void uct_ze_ipc_complete_event(uct_iface_h tl_iface,
                                        uct_ze_event_desc_t *ze_event)
{
    uct_ze_ipc_iface_t *iface               = ucs_derived_of(tl_iface,
                                                               uct_ze_ipc_iface_t);
    uct_ze_ipc_event_desc_t *ze_ipc_event = ucs_derived_of(ze_event,
                                                               uct_ze_ipc_event_desc_t);
    ucs_status_t status;

    status = uct_ze_ipc_unmap_memhandle(ze_ipc_event->pid,
                                          ze_ipc_event->d_bptr,
                                          ze_ipc_event->mapped_addr,
                                          ze_ipc_event->ze_device,
                                          iface->config.enable_cache); // todo:  we need to use L0 API
    if (status != UCS_OK) {
        ucs_fatal("failed to unmap addr:%p", ze_ipc_event->mapped_addr);
    }
}
...
static uct_iface_ops_t uct_ze_ipc_iface_ops = {
    ...
    .iface_flush              = uct_ze_base_iface_flush,
    .iface_fence              = uct_base_iface_fence,
    .iface_progress_enable    = uct_base_iface_progress_enable,
    .iface_progress_disable   = uct_base_iface_progress_disable,
    .iface_progress           = uct_ze_base_iface_progress,
    .iface_event_fd_get       = uct_ze_base_iface_event_fd_get,
    .iface_event_arm          = uct_ze_base_iface_event_fd_arm,
...
static ucs_status_t
uct_ze_ipc_estimate_perf(uct_iface_h tl_iface, uct_perf_attr_t *perf_attr) // todo: no need to estimate perf?
{
    return UCS_OK;
}

static ucs_status_t
uct_ze_ipc_iface_mem_element_pack(uct_iface_h tl_iface,
                                    uct_mem_h memh,
                                    uct_rkey_t rkey,
                                    uct_device_mem_element_t *mem_element)
{
    ...
    ucs_status_t status;
    int is_ctx_pushed;
    uct_ze_ipc_device_mem_element_t ze_ipc_mem_element;
    CUdevice ze_device; // todo: change to L0 device
    void *mapped_addr;

    status = uct_ze_ipc_check_and_push_ctx((CUdeviceptr)mem_element,
                                             &ze_device, &is_ctx_pushed);
    if (ucs_unlikely(status != UCS_OK)) {
        return status;
    }

    status = uct_ze_ipc_map_memhandle(&key->super, ze_device,
                                        &mapped_addr, UCS_LOG_LEVEL_ERROR);
    if (ucs_unlikely(status != UCS_OK)) {
        goto out;
    }
    ze_ipc_mem_element.mapped_offset = (ptrdiff_t)mapped_addr - (ptrdiff_t)key->super.d_bptr;

    status = UCT_CUDADRV_FUNC_LOG_ERR(
            cuMemcpyHtoD((CUdeviceptr)mem_element, &ze_ipc_mem_element,
                          sizeof(uct_ze_ipc_device_mem_element_t))); // todo: why need to change Host to device?
...
static ucs_status_t uct_ze_ipc_iface_query_v2(uct_iface_h iface,
                                                uct_iface_attr_v2_t *iface_attr)
{
    if (iface_attr->field_mask & UCT_IFACE_ATTR_FIELD_DEVICE_MEM_ELEMENT_SIZE) {
        iface_attr->device_mem_element_size = sizeof(uct_ze_ipc_device_mem_element_t);
    }
...
static uct_iface_internal_ops_t uct_ze_ipc_iface_internal_ops = {
    .iface_query_v2         = uct_ze_ipc_iface_query_v2,
    .iface_estimate_perf    = uct_ze_ipc_estimate_perf,
    .iface_vfs_refresh      = (uct_iface_vfs_refresh_func_t)ucs_empty_function,
    .iface_mem_element_pack = uct_ze_ipc_iface_mem_element_pack,
    ...
    .ep_get_device_ep       = uct_ze_ipc_ep_get_device_ep
};

static uct_ze_ctx_rsc_t * uct_ze_ipc_ctx_rsc_create(uct_iface_h tl_iface)
{
    uct_ze_ipc_iface_t *iface = ucs_derived_of(tl_iface, uct_ze_ipc_iface_t);
    uct_ze_ipc_ctx_rsc_t *ctx_rsc;
    unsigned i;

    ctx_rsc = ucs_malloc(sizeof(*ctx_rsc), "uct_ze_ipc_ctx_rsc_t");
    if (ctx_rsc == NULL) {
        ucs_error("failed to allocate ze ipc context resource struct");
        return NULL;
    }

    for (i = 0; i < iface->config.max_streams; i++) {
        uct_ze_base_queue_desc_init(&ctx_rsc->queue_desc[i]);
    }

    return &ctx_rsc->super;
}

static void uct_ze_ipc_ctx_rsc_destroy(uct_iface_h tl_iface,
                                         uct_ze_ctx_rsc_t *ze_ctx_rsc)
{
    uct_ze_ipc_iface_t *iface     = ucs_derived_of(tl_iface, uct_ze_ipc_iface_t);
    uct_ze_ipc_ctx_rsc_t *ctx_rsc = ucs_derived_of(ze_ctx_rsc, uct_ze_ipc_ctx_rsc_t);
    unsigned i;

    for (i = 0; i < iface->config.max_streams; i++) {
        uct_ze_base_queue_desc_destroy(ze_ctx_rsc, &ctx_rsc->queue_desc[i]);
    }

    ucs_free(ctx_rsc);
}

static uct_ze_iface_ops_t uct_ze_iface_ops = {
    .create_rsc     = uct_ze_ipc_ctx_rsc_create,
    .destroy_rsc    = uct_ze_ipc_ctx_rsc_destroy,
    .complete_event = uct_ze_ipc_complete_event
};
...
{
    uct_ze_ipc_iface_config_t *config = NULL;
    ucs_status_t status;

    config = ucs_derived_of(tl_config, uct_ze_ipc_iface_config_t);
    UCS_CLASS_CALL_SUPER_INIT(uct_ze_iface_t, &uct_ze_ipc_iface_ops,
                              &uct_ze_ipc_iface_internal_ops, md, worker, params,
                              tl_config, "ze_ipc");

    status = uct_ze_base_check_device_name(params);
    if (status != UCS_OK) {
        return status;
    }
...
//    }
    ...
    self->super.ops                    = &uct_ze_iface_ops;
    self->super.config.max_events      = self->config.max_ze_ipc_events;
    self->super.config.max_poll        = self->config.max_poll;
    self->super.config.event_desc_size = sizeof(uct_ze_ipc_event_desc_t);
    return UCS_OK;
}
...
UCS_CLASS_DEFINE(uct_ze_ipc_iface_t, uct_ze_iface_t);
...
UCT_TL_DEFINE(&uct_ze_ipc_component.super, ze_ipc,
