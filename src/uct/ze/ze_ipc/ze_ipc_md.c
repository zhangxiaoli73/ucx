...
//#include <uct/ze/base/cuda_nvml.h> //todo: no need
...
static ucs_config_field_t uct_ze_ipc_md_config_table[] = {
    {"", "", NULL,
     ucs_offsetof(uct_ze_ipc_md_config_t, super), UCS_CONFIG_TYPE_TABLE(uct_md_config_table)},

    {"ENABLE_MNNVL", "try",
     "Enable multi-node NVLINK capabilities.",
     ucs_offsetof(uct_ze_ipc_md_config_t, enable_mnnvl), UCS_CONFIG_TYPE_TERNARY},

    {NULL}
};

static uct_ze_ipc_dev_cache_t *uct_ze_ipc_create_dev_cache(int dev_num)
{
    uct_ze_ipc_dev_cache_t *cache;
    ucs_status_t status;
    int i, num_devices;

    status = cuDeviceGetCount(&num_devices); // todo: check?
    if (UCS_OK != status) {
        return NULL;
    }
    ...
    if (cache == NULL) {
        ucs_error("failed to allocate memory for uct_ze_ipc_dev_cache_t");
        return NULL;
    }
...
static uct_ze_ipc_dev_cache_t *
uct_ze_ipc_get_dev_cache(uct_ze_ipc_component_t *component,
                           uct_ze_ipc_rkey_t *rkey)
{
    khash_t(ze_ipc_uuid_hash) *hash = &component->uuid_hash;
    uct_ze_ipc_uuid_hash_key_t key;
    uct_ze_ipc_dev_cache_t *cache;
    khiter_t iter;
    int ret;
    ...
    iter = kh_put(ze_ipc_uuid_hash, hash, key, &ret);
    if (ret == UCS_KH_PUT_KEY_PRESENT) {
        return kh_val(hash, iter);
    } else if ((ret == UCS_KH_PUT_BUCKET_EMPTY) ||
               (ret == UCS_KH_PUT_BUCKET_CLEAR)) {
        cache = uct_ze_ipc_create_dev_cache(kh_size(hash) - 1);
        if (NULL != cache) {
            kh_val(hash, iter) = cache;
        }
        return cache;
    } else {
        ucs_error("kh_put(ze_ipc_uuid_hash) failed with %d", ret);
        return NULL;
    }
}
...
static ucs_status_t
uct_ze_ipc_mem_add_reg(void *addr, uct_ze_ipc_memh_t *memh,
                         uct_ze_ipc_lkey_t **key_p)
{
    uct_ze_ipc_lkey_t *key;
    ucs_status_t status;
    int is_ctx_pushed;
    CUdevice ze_device;
    // todo: no need for XPU
#if HAVE_CUDA_FABRIC
#define UCT_CUDA_IPC_QUERY_NUM_ATTRS 3
    ...
    key = ucs_calloc(1, sizeof(*key), "uct_ze_ipc_lkey_t");
    if (key == NULL) {
        return UCS_ERR_NO_MEMORY;
    }

    status = uct_ze_ipc_check_and_push_ctx((CUdeviceptr)addr, &ze_device,
                                             &is_ctx_pushed);
    if (status != UCS_OK) {
        goto out;
    }

    status = cuMemGetAddressRange(&key->d_bptr, &key->b_len, (CUdeviceptr)addr);
    if (status != UCS_OK) {
        goto out_pop_ctx;
    }

    status = cuPointerGetAttribute(&key->ph.buffer_id,
    ...
    status = cuPointerGetAttributes(ucs_static_array_size(attr_data), attr_type,
    ...
    status = cuMemRetainAllocationHandle(&handle, addr);
    if (status == UCS_OK) {
        ...
        key->ph.handle_type = UCT_CUDA_IPC_KEY_HANDLE_TYPE_VMM; // todo: what is it on XPU?
    ...
    status = cuMemPoolExportToShareableHandle(
    ...
    status = cuMemPoolExportPointer(&key->ph.ptr, (CUdeviceptr)key->d_bptr);
    if (status != UCS_OK) {
        goto out_pop_ctx;
    }

    key->ph.handle_type = UCT_CUDA_IPC_KEY_HANDLE_TYPE_MEMPOOL; // todo: no memory pool supported?
...
legacy_path:
    ...
    status              = cuIpcGetMemHandle(&key->ph.handle.legacy, (CUdeviceptr)addr); // todo: could be XPU path?
    if (status != UCS_OK) {
        goto out_pop_ctx;
    }
...
common_path:
    ...
    ucs_trace("registered addr:%p/%p length:%zd type:%u dev_num:%d "
              ...
              ze_device, key->ph.buffer_id);

    memh->dev_num = ze_device;
    *key_p        = key;
    status        = UCS_OK;
...
static ucs_status_t
uct_ze_ipc_mkey_pack(uct_md_h md, uct_mem_h tl_memh, void *address,
                       size_t length, const uct_md_mkey_pack_params_t *params,
                       void *mkey_buffer)
{
    uct_ze_ipc_rkey_t *packed = mkey_buffer;
    uct_ze_ipc_memh_t *memh   = tl_memh;
    uct_ze_ipc_lkey_t *key;
    ucs_status_t status;
    ...
    status = uct_ze_ipc_mem_add_reg(address, memh, &key);
    if (status != UCS_OK) {
        return status;
    }
...
found:
    ...
    return cuDeviceGetUuid(&packed->uuid, memh->dev_num);
}

static ucs_status_t
uct_ze_ipc_is_peer_accessible(uct_ze_ipc_component_t *component,
                                uct_ze_ipc_unpacked_rkey_t *rkey,
                                ucs_sys_device_t sys_dev)
{
    ...
    if (sys_dev == UCS_SYS_DEVICE_ID_UNKNOWN) {
        ...
        status = uct_ze_base_get_ze_device(sys_dev, &cu_dev);
        if (status != UCS_OK) {
            ucs_warn("failed to map sys device [%d] to ze device: %s",
    ...
    cache = uct_ze_ipc_get_dev_cache(component, &rkey->super);
    ...
    if (ucs_unlikely(*accessible == UCS_TRY)) { /* unchecked, add to cache */
        ...
        status = uct_ze_ipc_map_memhandle(&rkey->super, cu_dev, &d_mapped,
                                            UCS_LOG_LEVEL_DEBUG);
...
UCS_PROFILE_FUNC(ucs_status_t, uct_ze_ipc_rkey_unpack,
                 ...
                 void **handle_p)
{
    uct_ze_ipc_component_t *com = ucs_derived_of(component,
                                                   uct_ze_ipc_component_t);
    uct_ze_ipc_rkey_t *packed   = (uct_ze_ipc_rkey_t *)rkey_buffer;
    ...
    if (NULL == unpacked) {
        ucs_error("failed to allocate memory for uct_ze_ipc_unpacked_rkey_t");
    ...
    status = uct_ze_ipc_is_peer_accessible(com, unpacked, sys_dev);
    if (status != UCS_OK) {
        goto err_free_key;
    }
...
static ucs_status_t
uct_ze_ipc_mem_reg(uct_md_h md, void *address, size_t length,
                     const uct_md_mem_reg_params_t *params, uct_mem_h *memh_p)
{
    uct_ze_ipc_memh_t *memh;

    memh = ucs_malloc(sizeof(*memh), "uct_ze_ipc_memh_t");
    if (NULL == memh) {
        ucs_error("failed to allocate memory for uct_ze_ipc_memh_t");
        return UCS_ERR_NO_MEMORY;
    }

    /* dev_num is initialized during pack in uct_ze_ipc_mem_add_reg */
...
static ucs_status_t
uct_ze_ipc_mem_dereg(uct_md_h md, const uct_md_mem_dereg_params_t *params)  // todo: why it's not need for copy part?
{
    uct_ze_ipc_memh_t *memh = params->memh;
    uct_ze_ipc_lkey_t *key, *tmp;
...
//static int
//uct_ze_ipc_md_check_fabric_info(uct_ze_ipc_md_t *md,
...
//    }
//
//    status = UCT_CUDA_NVML_WRAP_CALL(nvmlDeviceGetHandleByIndex, 0, &device); // todo: no corresponding API
//    if (status != UCS_OK) {
...
static void uct_ze_ipc_md_close(uct_md_h md)
{
    ucs_free(md);
}

static ucs_status_t
uct_ze_ipc_md_open(uct_component_t *component, const char *md_name,
                     const uct_md_config_t *config, uct_md_h *md_p)
{
    ...
    uct_ze_ipc_md_config_t *ipc_config = ucs_derived_of(config,
                                                          uct_ze_ipc_md_config_t);
    uct_ze_ipc_md_t* md;

    md = ucs_calloc(1, sizeof(*md), "uct_ze_ipc_md");
    if (md == NULL) {
        return UCS_ERR_NO_MEMORY;
    }

    md->super.ops       = &md_ops;
    md->super.component = &uct_ze_ipc_component.super;
    md->enable_mnnvl    = 0; //zl_debug: no need mnnvl on PCIe BMG
    *md_p               = &md->super;

    return UCS_OK;
}

uct_ze_ipc_component_t uct_ze_ipc_component = {
    .super = {
        ...
        .tl_list            = UCT_COMPONENT_TL_LIST_INITIALIZER(&uct_ze_ipc_component.super),
        .flags              = 0,
        .md_vfs_init        =
...
UCT_COMPONENT_REGISTER(&uct_ze_ipc_component.super);

UCS_STATIC_CLEANUP {
    uct_ze_ipc_dev_cache_t *cache;

    kh_foreach_value(&uct_ze_ipc_component.uuid_hash, cache, {
        ucs_free(cache);
    })
    kh_destroy_inplace(ze_ipc_uuid_hash, &uct_ze_ipc_component.uuid_hash);
    pthread_mutex_destroy(&uct_ze_ipc_component.lock);
}
