...
ucs_status_t ucp_worker_mem_type_eps_create(ucp_worker_h worker)
{
    ...
    ucs_memory_type_for_each(mem_type) {
        ...
        UCS_ASYNC_BLOCK(&worker->async);
        printf("[UCX/UCP/EP] zl_debug start to ucp_ep_create_to_worker_addr with memoty type %d \n", mem_type);
        ...
        if (status != UCS_OK) {
            printf("[UCX/UCP/EP] zl_debug failed to ucp_ep_create_to_worker_addr");
            UCS_ASYNC_UNBLOCK(&worker->async);
            goto err_free_address_list;
        } else {
            printf("[UCX/UCP/EP] zl_debug passed to ucp_ep_create_to_worker_addr");
        }
